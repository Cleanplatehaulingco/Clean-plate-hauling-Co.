...
import {
  Search, Phone, MapPin, Plus, Database, RefreshCw, Calculator,
} from "lucide-react";
...
export default function Pipeline() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Stage updated", description: "Pipeline and Google Sheets CRM are synced." });
    },
    onError: () => {
      toast({ title: "Sync failed", description: "The stage did not write to Google Sheets. Try refresh and update again.", variant: "destructive" });
    },
  });
      ...
      {view === "board" ? (
            ...
            return (
              <div key={stage.key} className="rounded-lg border border-card-border bg-card p-4" data-testid={`column-${stage.key}`}>
                ...
                <div className="space-y-2">
                  ...
                  {stageLeads.map(lead => (
                    <LeadCard key={lead.id} lead={lead} onChangeStage={(stage) => updateMutation.mutate({ id: lead.id, stage })} />
                  ))}
                </div>
              </div>
            );
        ...
        <div className="rounded-lg border border-card-border bg-card overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map(lead => (
                <tr key={lead.id} className="border-t border-border hover-elevate" data-testid={`row-lead-${lead.id}`}>
                  ...
                  <td className="px-4 py-3">
                    <Select value={lead.stage} onValueChange={(stage) => updateMutation.mutate({ id: lead.id, stage })}>
                      <SelectTrigger className="h-8 text-xs w-[150px]" data-testid={`select-stage-${lead.id}`}><SelectValue /></SelectTrigger>
...
function LeadCard({ lead, onChangeStage }: { lead: PipelineLead; onChangeStage: (stage: string) => void }) {
  return (
    <div className="rounded-md border border-border bg-background/60 p-3 hover-elevate" data-testid={`card-lead-${lead.id}`}>
