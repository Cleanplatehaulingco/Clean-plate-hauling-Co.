...
const require = createRequire(`${process.cwd()}/server/storage.ts`);
let sqlite: any = null;
let db: any = null;
try {
  const { drizzle } = require("drizzle-orm/better-sqlite3");
  const Database = require("better-sqlite3");
  const sqliteFile = process.env.SQLITE_FILE || "data.db";
  if (sqliteFile !== ":memory:") {
    require("node:fs").mkdirSync(path.dirname(path.resolve(sqliteFile)), { recursive: true });
  }
  sqlite = new Database(sqliteFile);
  sqlite.pragma("journal_mode = WAL");
  db = drizzle(sqlite);
} catch (err: any) {
  console.warn("SQLite storage unavailable; using in-memory fallback.", err?.message ?? err);
}
...
class MemoryStorage implements IStorage {
  ...
  private jobs: any[] = [];
  private crews: any[] = [
    { id: 1, name: "Alpha Crew", lead: "Marcus Webb", truck: "Truck 1 — Ram 5500 dump", capacityYards: 15, phone: "248-555-0142", active: true },
    { id: 2, name: "Bravo Crew", lead: "Tyrell Jackson", truck: "Truck 2 — F-450 dump", capacityYards: 12, phone: "248-555-0177", active: true },
    { id: 3, name: "Solo Run", lead: "Owner (You)", truck: "Truck 1 — Ram 5500 dump", capacityYards: 15, phone: "248-555-0100", active: true },
  ];
  private estimates: any[] = [];
  private settings: any = { id: 1, businessName: "Clean Plate Hauling Co", homeBase: "Wixom, MI", dumpFeePerLoad: 85, laborHourlyRate: 45, travelFeePerMile: 1.75, minimumJobPrice: 125, targetMarginPct: 55, crewCapacityYards: 15, stairsFee: 25, heavyItemFee: 35, baseTruckFee: 95, pricePerYard: 38, autoOptimizeMode: true, maxAutoPriceAdjustPct: 8, followUpSpeedHours: 2, lastOptimizedAt: null };
  private followUps: any[] = [
    { id: 1, kind: "quote_sent", channel: "text", title: "Quote sent — 24 hour check-in", body: "Hey {{name}}, this is {{owner}} from Clean Plate Hauling. Just checking in on the quote we sent for your {{jobType}} in {{city}}." },
    { id: 2, kind: "missed_call", channel: "text", title: "Missed call — same-day text", body: "Hey, this is Clean Plate Hauling Co — sorry I missed your call. Text me what you're trying to get hauled and your city and I'll get you a same-day quote." },
  ];
  private receipts: any[] = [];
  private invoices: any[] = [];
  private builderActions: any[] = [];
  private businessMemory: any[] = [];
  private memoryEvents: any[] = [];
  private agents: any[] = [];
  private next(key: string) { this.ids[key] = (this.ids[key] ?? 0) + 1; return this.ids[key]; }
  async listLeads() { return this.leads; }
  async getLead(id: number) { return this.leads.find(x => x.id === id); }
  async createLead(data: InsertLead) { const now = nowIso(); const row = { ...data, id: this.next("leads"), createdAt: now, updatedAt: now }; this.leads.unshift(row); return row as any; }
  async updateLead(id: number, data: Partial<InsertLead>) { const row = await this.getLead(id); if (!row) return undefined; Object.assign(row, data, { updatedAt: nowIso() }); return row as any; }
  async deleteLead(id: number) { this.leads = this.leads.filter(x => x.id !== id); }
  async listJobs() { return this.jobs; }
  async getJob(id: number) { return this.jobs.find(x => x.id === id); }
  async createJob(data: InsertJob) { const row = { ...data, id: this.next("jobs"), createdAt: nowIso() }; this.jobs.unshift(row); return row as any; }
  async updateJob(id: number, data: Partial<InsertJob>) { const row = await this.getJob(id); if (!row) return undefined; Object.assign(row, data); return row as any; }
  async listCrews() { return this.crews; }
  async listEstimates() { return this.estimates; }
  async createEstimate(data: InsertEstimate) { const row = { ...data, id: this.next("estimates"), createdAt: nowIso() }; this.estimates.unshift(row); return row as any; }
  async getSettings() { return this.settings as any; }
  async updateSettings(data: Partial<InsertSettings>) { Object.assign(this.settings, data); return this.settings as any; }
  async listFollowUps() { return this.followUps; }
  async listReceipts() { return this.receipts; }
  async createReceipt(data: InsertReceipt) { const id = this.next("receipts"); const receiptNumber = `CP-${new Date().toISOString().slice(0,10).replace(/-/g, "")}-${String(id).padStart(3, "0")}`; const row = { ...data, id, receiptNumber, createdAt: nowIso() }; this.receipts.unshift(row); return row as any; }
  async listInvoices() { return this.invoices; }
  async getInvoice(id: number) { return this.invoices.find(x => x.id === id); }
  async createInvoice(data: InsertInvoice) { const id = this.next("invoices"); const row = { ...data, id, invoiceNumber: `CP-INV-${String(id).padStart(3, "0")}`, status: (data as any).status ?? "draft", createdAt: nowIso(), updatedAt: nowIso() }; this.invoices.unshift(row); return row as any; }
  async updateInvoice(id: number, data: Partial<InsertInvoice> & { status?: string; sentAt?: string | null }) { const row = await this.getInvoice(id); if (!row) return undefined; Object.assign(row, data, { updatedAt: nowIso() }); return row as any; }
  async listBuilderActions() { return this.builderActions; }
  async createBuilderAction(data: any) { const row = { ...data, id: this.next("builderActions"), input: data.input ?? {}, preview: data.preview ?? {}, createdAt: nowIso() }; this.builderActions.unshift(row); return row; }
  async updateBuilderAction(id: number, data: any) { const row = this.builderActions.find(x => x.id === id); if (!row) return undefined; Object.assign(row, data); return row; }
  async listBusinessMemory() { return this.businessMemory; }
  async upsertBusinessMemory(data: any) { const existing = this.businessMemory.find(x => x.memoryType === data.memoryType && x.key === data.key); if (existing) { Object.assign(existing, data, { updatedAt: nowIso() }); return existing; } const row = { ...data, id: this.next("businessMemory"), createdAt: nowIso(), updatedAt: nowIso() }; this.businessMemory.unshift(row); return row; }
  async listMemoryEvents(limit = 200) { return this.memoryEvents.slice(0, limit); }
  async createMemoryEvent(data: InsertAiMemoryEvent) { const row = { ...data, id: this.next("memoryEvents"), createdAt: nowIso() }; this.memoryEvents.unshift(row); return row as any; }
  async searchMemoryEvents(query: string, limit = 50) { const q = String(query ?? "").toLowerCase(); return this.memoryEvents.filter(x => `${x.title} ${x.summary} ${x.category} ${x.customerName ?? ""}`.toLowerCase().includes(q)).slice(0, limit); }
  async listAgents() { return this.agents; }
  async createAgent(data: InsertAiAgent) { const row = { ...data, id: this.next("agents"), createdAt: nowIso(), updatedAt: nowIso() }; this.agents.unshift(row); return row as any; }
  async updateAgent(id: number, data: Partial<InsertAiAgent>) { const row = this.agents.find(x => x.id === id); if (!row) return undefined; Object.assign(row, data, { updatedAt: nowIso() }); return row as any; }
}

export const storage: IStorage = sqlite ? new DatabaseStorage() : new MemoryStorage();
