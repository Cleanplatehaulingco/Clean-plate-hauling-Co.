...
export async function registerRoutes(
  ...
  function invoiceEmail(invoice: any) {
    ...
    const subject = invoice.emailSubject || `Clean Plate Hauling Co invoice ${invoice.invoiceNumber}`;
    const body = invoice.emailBody || [
      `Hi ${invoice.customerName},`,
      "",
      "Thank you for choosing Clean Plate Hauling Co. Your branded invoice summary is below.",
      "",
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      "CLEAN PLATE HAULING CO",
      "Veteran-owned junk removal · Wixom + Metro Detroit",
      "cleanplatehaulingco.com · 734-743-1877",
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
      "",
      `Invoice #: ${invoice.invoiceNumber}`,
      invoice.dueDate ? `Due date: ${invoice.dueDate}` : "Due date: Due on receipt",
      invoice.address || invoice.city ? `Service location: ${[invoice.address, invoice.city].filter(Boolean).join(", ")}` : "",
      "",
      "Services",
      lines,
      "",
      `Subtotal: ${moneyCell(invoice.subtotal)}`,
      Number(invoice.tax || 0) ? `Tax: ${moneyCell(invoice.tax)}` : "Tax: $0",
      `TOTAL DUE: ${moneyCell(invoice.total)}`,
      invoice.notes ? `Notes: ${invoice.notes}` : "",
      "",
      "Reply directly to this email with questions, access notes, or payment confirmation.",
      "",
      "Thank you,",
      "Clean Plate Hauling Co",
    ].filter(Boolean).join("\n");
  ...
  app.patch("/api/invoices/:id", async (req, res) => {
    ...
    const updated = await storage.updateInvoice(id, {
      email: input.email !== undefined ? input.email || null : invoice.email,
      emailSubject: input.emailSubject !== undefined ? input.emailSubject : invoice.emailSubject,
      emailBody: input.emailBody !== undefined ? input.emailBody : invoice.emailBody,
